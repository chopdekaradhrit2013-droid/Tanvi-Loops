  ...
  return (
    <section className="page product-layout">
      ...
      <div>
        ...
        {soldOut ? <p className="sold-text">Sold out</p> : <p>In stock · {product.stock} remaining</p>}
        ...
        <div className="qty">
          <button onClick={() => setQty((n) => Math.max(1, n - 1))} disabled={soldOut}>−</button>
          <span>{qty}</span>
          <button onClick={() => setQty((n) => n + 1)} disabled={soldOut}>+</button>
        </div>
        <div className="actions">
          <button className="pill-btn dark" disabled={soldOut} onClick={() => addToCart(product, qty, color)}>{soldOut ? 'Sold out' : 'Add to Cart'}</button>
          {!soldOut && <button className="pill-btn" onClick={() => { addToCart(product, qty, color); nav('/checkout') }}>Buy Now</button>}
