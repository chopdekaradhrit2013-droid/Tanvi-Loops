...
export default function Checkout() {
  ...
  const [form, setForm] = useState({ name: user?.name || '', email: user?.email || '', address: '', city: '', payment: 'Card' })
  if (!cartItems.length) return <section className="page"><p>Your cart is empty.</p></section>
  const submit = (e) => { e.preventDefault(); const order = placeOrder(form); nav(`/confirmation/${order.id}`) }
  return (
    <section className="page checkout-wrap">
      <div>
        ...
        <form className="form" onSubmit={submit}>
          ...
          <select value={form.payment} onChange={(e) => setForm({ ...form, payment: e.target.value })}>
            <option>Card</option><option>PayPal</option><option>Apple Pay</option>
          </select>
      ...
      <div>
        ...
        <div className="totals">
          ...
          <div className="row"><strong>Total</strong><strong>${(subtotal + shipping).toFixed(2)}</strong></div>
